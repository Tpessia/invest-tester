...
export function getNextBusinessDay(date: Date = new Date()): Date {
    const nextDay = new Date(date); // Create a new Date object to avoid modifying the input
    nextDay.setDate(nextDay.getDate() + 1); // Add one day
    while (nextDay.getDay() === 0 || nextDay.getDay() === 6) // Keep adding days until we get a business day (Monday-Friday)
        nextDay.setDate(nextDay.getDate() + 1);
    return nextDay;
}

export function averageDatesPerYear(dates: Date[]): number | null {
    if (dates.length === 0) return null;

    const yearData = new Map<number, { count: number; minDate: Date; maxDate: Date }>();

    // Group dates by year, count them, and track min and max dates
    for (const date of dates) {
        const year = date.getFullYear();
        const currentData = yearData.get(year) || { count: 0, minDate: date, maxDate: date };
        yearData.set(year, {
            count: currentData.count + 1,
            minDate: date < currentData.minDate ? date : currentData.minDate,
            maxDate: date > currentData.maxDate ? date : currentData.maxDate
        });
    }

    let totalAdjustedCount = 0;

    // Adjust based on the coverage of each year (e.g. partial years)
    for (const [year, { count, minDate, maxDate }] of yearData.entries()) {
        const daysCovered = Math.floor((maxDate.getTime() - minDate.getTime()) / (1000 * 60 * 60 * 24)) + 1;
        const totalDaysInYear = new Date(year, 11, 31).getTime() - new Date(year, 0, 1).getTime();

        const adjustedCount = Math.round((count / daysCovered) * (totalDaysInYear / (1000 * 60 * 60 * 24)));
        totalAdjustedCount += adjustedCount;
    }

    // Calculate and return the average
    return Math.round(totalAdjustedCount / yearData.size);
}
